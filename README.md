...
* **Automated Regression Testing:** Includes a full `pytest` suite for asserting security blocks and functional integration paths.
...
pip install langgraph langchain langchain-community langchain-ollama langchain-chroma chromadb pydantic python-dotenv pytest
```
...
* **Anti-Hallucination Writer:** Stripped creative generation defaults, forcing the LLM to output a safe "NO DATA FOUND" response when citations are missing.

### Architectural Improvements
* **Semantic Routing:** Overhauled hardcoded regex matching with semantic intent extraction for jurisdictions.
